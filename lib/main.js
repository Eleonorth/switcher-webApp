...
function defaultCidr(ip1, ip2,cidr) {
...
$(document).ready(function(){
    ...
    $('#calc').click(function( event ) {
        ...
        var ip4 = $('#ip4').val();
        var cidr = $('#cidr').val();
        cidr = defaultCidr(ip1,ip2,cidr);
        console.log(cidr);

        
        // On vérifie si l'utilisateur a bien rentré une adresse IP et si la taille des octets est bonne
        isEmpty(ip1,ip2,ip3,ip4);
        octetLength(ip1,ip2,ip3,ip4, cidr); 

        VerifNbr(ip1+ip2+ip3+ip4+cidr);


        
        // Si l'utilisateur s'est trompé, on remet la variable isAlerted sur false, pour relancer les fonctions
        if (isAlerted == true) {
        ...
        if (isPrivate(ip1,ip2) == true) {
            ...
            console.log("Il y a "+NbHote+" machines disponibles");
